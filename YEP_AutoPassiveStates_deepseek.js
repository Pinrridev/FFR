...
// Versión Completa Modificada con Passive State Turns funcional
//=============================================================================
...
Yanfly.APS.version = '1.17.2';

//=============================================================================
/*:
 * @plugindesc v1.17.1 This plugin allows for some states to function as
 * passives for actors, enemies, skills, and equips. Now with timed passives.
 ...
 * @help
 * ============================================================================
 * New Features in Modified Version
 * ============================================================================
 *
 * Added timed passive states functionality:
 *
 * Use the following notetag to create temporary passive states:
 *   <Passive State Turns: x, y>
 *   Where x is the state ID and y is the number of turns it should last
 *
 * Example:
 *   <Passive State Turns: 12, 3>  // State 12 will last for 3 turns
 *
 * ============================================================================
 * Original Documentation
 * ============================================================================
 * [Rest of original help text...]
 *
//=============================================================================
 * ============================================================================
 * Introduction
...
Yanfly.SetupParameters = function() {
  ...
  for (var i = 0; i < Yanfly.Param.APSGlobalPas.length; ++i) {
    var id = parseInt(Yanfly.Param.APSGlobalPas[i]);
...
DataManager.processAPSNotetags1 = function(group, inheritArray) {
  ...
  var note3 = /<(?:PASSIVE STATE TURNS):[ ]*(\d+)[ ]*,[ ]*(\d+)>/i;
  ...
  for (var n = 1; n < group.length; n++) {
    ...
    for (var i = 0; i < notedata.length; i++) {
      ...
      if (line.match(note1)) {
        ...
        obj.passiveStateTurns[stateId] = turns;
      }
    }
  }
};
...
DataManager.processAPSNotetags2 = function(group) {
  ...
  for (var n = 1; n < group.length; n++) {
    ...
    obj.passiveCondition = new Function('condition', 'a', 'user', 'subject', 'b', 'target', 's', 'v', 
      obj.passiveCondition + '\nreturn condition;');
    obj.passiveConditionEval = new Function('condition', 'a', 'user', 'subject', 'b', 'target', 's', 'v', 
      obj.passiveConditionEval + '\nreturn condition;');
  }
};
...
Game_BattlerBase.prototype.passiveStatesRaw = function() {
    ...
    var array = [];
    // Añadir estados temporales activos
    var turns = this.passiveStateTurns();
    for (var stateId in turns) {
        if (turns.hasOwnProperty(stateId) && turns[stateId] > 0) {
            if (!array.contains(parseInt(stateId))) {
                array.push(parseInt(stateId));
            }
        }
    }
    
    // Añadir estados pasivos normales (no temporales)
    if (this.getNormalPassiveStates) {
        var normalPassives = this.getNormalPassiveStates();
        for (var i = 0; i < normalPassives.length; i++) {
            var stateId = normalPassives[i];
            if (!array.contains(stateId)) {
                // Verificar que no sea un estado temporal ya aplicado
                if (!turns[stateId]) {
                    array.push(stateId);
                }
            }
...
Game_BattlerBase.prototype.passiveStateTurns = function() {
    this._passiveStateTurns = this._passiveStateTurns || {};
    return this._passiveStateTurns;
};

Game_BattlerBase.prototype.updatePassiveStateTurns = function() {
    var turns = this.passiveStateTurns();
    var needsRefresh = false;
    
    for (var stateId in turns) {
        if (turns.hasOwnProperty(stateId)) {
            turns[stateId]--;
            if (turns[stateId] <= 0) {
                delete turns[stateId];
                needsRefresh = true;
            }
        }
    }
...
Game_BattlerBase.prototype.getPassiveStateData = function(obj) {
    if (!obj) return [];
    if (!obj.passiveStates && !obj.passiveStateTurns) return [];
    
    var array = [];
    // Procesar estados pasivos normales
    if (obj.passiveStates) {
    ...
    if (obj.passiveStateTurns) {
        for (var stateId in obj.passiveStateTurns) {
            if (obj.passiveStateTurns.hasOwnProperty(stateId)) {
                var turns = obj.passiveStateTurns[stateId];
                if (this.meetPassiveStateCondition(parseInt(stateId))) {
                    this.passiveStateTurns()[stateId] = turns;
                }
            }
        }
    }
...
Yanfly.APS.Game_Battler_onBattleEnd = Game_Battler.prototype.onBattleEnd;
Game_Battler.prototype.onBattleEnd = function() {
    Yanfly.APS.Game_Battler_onBattleEnd.call(this);
    this._passiveStateTurns = {};
    this._passiveStatesRaw = undefined;
    this.refresh();
};
...
Game_Actor.prototype.getNormalPassiveStates = function() {
    var array = [];
    ...
    for (var i = 0; i < this.equips().length; ++i) {
        ...
        // Añadir estados normales del equipo (no temporales)
        if (equip.passiveStates) {
            for (var j = 0; j < equip.passiveStates.length; j++) {
                var stateId = equip.passiveStates[j];
                if (!equip.passiveStateTurns || !equip.passiveStateTurns[stateId]) {
                    if (this.meetPassiveStateCondition(stateId)) {
                        array.push(stateId);
                    }
                }
            }
    ...
    for (var i = 0; i < this._skills.length; ++i) {
        ...
        // Añadir estados normales de habilidades (no temporales)
        if (skill.passiveStates) {
            for (var j = 0; j < skill.passiveStates.length; j++) {
                var stateId = skill.passiveStates[j];
                if (!skill.passiveStateTurns || !skill.passiveStateTurns[stateId]) {
                    if (this.meetPassiveStateCondition(stateId)) {
                        array.push(stateId);
                    }
                }
...
Game_Actor.prototype.passiveStatesRaw = function() {
    ...
    // Añadir datos de estados pasivos normales
    array = array.concat(this.getPassiveStateData(this.actor()));
    array = array.concat(this.getPassiveStateData(this.currentClass()));
...
Game_Enemy.prototype.getNormalPassiveStates = function() {
    var array = [];
...
Yanfly.APS.BattleManager_updateTurn = BattleManager.updateTurn;
BattleManager.updateTurn = function() {
    Yanfly.APS.BattleManager_updateTurn.call(this);
    
    // Actualizar estados pasivos temporales
    $gameParty.members().forEach(function(member) {
        member.updatePassiveStateTurns();
    });
...
Yanfly.APS.BattleManager_endTurn = BattleManager.endTurn;
BattleManager.endTurn = function() {
    Yanfly.APS.BattleManager_endTurn.call(this);
    
    // Forzar actualización de estados al finalizar turno
    $gameParty.members().forEach(function(member) {
        member._passiveStatesRaw = undefined;
    });
    $gameTroop.members().forEach(function(member) {
        member._passiveStatesRaw = undefined;
    });
    
    // Refrescar todos los miembros
    $gameParty.refreshMembers();
