...
Sprite_Enemy.prototype.addVisualATBWindow = function() {
    ...
    // Verificamos que `this._visualATBWindow` existe
    if (!this._visualATBWindow) return;

    // Validamos el contenedor principal
    const parent = this.parent;
    if (parent && parent.children.includes(this)) {
        // Si el sprite enemigo es hijo válido, añadimos la barra justo detrás
        const index = parent.getChildIndex(this);
        parent.addChildAt(this._visualATBWindow, index);

        // Establecer el Z-index de la barra de ATB para que se dibuje detrás de la barra de HP
        this._visualATBWindow.z = -1;  // Ajusta este valor según necesites, asegúrate de que sea menor que el Z-index de las barras de HP
    }
};
